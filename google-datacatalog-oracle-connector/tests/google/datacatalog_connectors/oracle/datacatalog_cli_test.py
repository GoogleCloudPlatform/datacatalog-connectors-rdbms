...
class DatacatalogCLITestCase(TestCase):
    ...
    def test_datacatalog_cli_run_should_not_raise_error(
            self, run, add_argument, parse_args):  # noqa:E125
        ...
        mocked_parse_args.oracle_pass = 'name'
        mocked_parse_args.oracle_db_service = 'XEPDB1'
        ...
        for call_arg in add_argument.call_args_list:
            ...
            if '--service-account-path' in command \
                    or '--oracle-port' in command \
                    or '--oracle-user' in command \
