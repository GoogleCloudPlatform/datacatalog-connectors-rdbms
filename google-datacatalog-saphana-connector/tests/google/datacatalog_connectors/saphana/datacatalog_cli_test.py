...
from google.datacatalog_connectors.saphana import datacatalog_cli
...
class DatacatalogCLITestCase(unittest.TestCase):
    ...
    def test_datacatalog_cli_run_should_not_raise_error(
        ...
        mocked_parse_args.saphana_host = 'host'
        mocked_parse_args.saphana_user = 'user'
        mocked_parse_args.saphana_pass = 'pass'
        mocked_parse_args.saphana_database = 'db'
        ...
        for call_arg in add_argument.call_args_list:
            ...
            if '--service-account-path' in command \
                    or '--saphana-user' in command \
                    or '--saphana-pass' in command \
                    or '--saphana-database' in command \
